...
import type { IPedidoComercianteResumen } from '../../types/IPedido';
import {
  formatearFecha,
  generarCodigoPedido,
  ESTADO_PEDIDO_INFO,
  pedidoCoincideConTab,
  type TabPedidos,
} from '../../utils/pedidoUi';
...
const TABS: { key: TabPedidos; label: string }[] = [
...
export default function TableroDePedidosPage() {
  const navigate = useNavigate();

  const [pedidos, setPedidos] = useState<IPedidoComercianteResumen[]>([]);
  ...
  const filtrados = useMemo(() => {
    ...
    return pedidos.filter((p) => {
      if (!pedidoCoincideConTab(p.estado, tab)) return false;
      if (!busquedaNorm) return true;
      const codigo = generarCodigoPedido(p.fecha, p.id).toLowerCase();
    ...
    <div className="flex min-h-screen">
      ...
      <main className="min-w-0 flex-1 bg-gray-100 px-4 py-16 sm:px-6 lg:p-7">
        <p className="text-[12px] text-gray-500 mb-2">
          Inicio &rsaquo; <span className="text-primario font-medium">Pedidos</span>
        </p>

        {/* Header */}
        <div className="flex items-center justify-between mb-5">
          <h1 className="text-[22px] font-bold text-gray-900">Tablero de Pedidos</h1>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          {cargando ? (
            ...
            <div className="flex items-center justify-center py-20 text-[13px] text-gray-400">
              {pedidos.length === 0 ? 'No tienes pedidos aún.' : 'Sin resultados para los filtros aplicados.'}
            </div>
          ) : (
            <table className="w-full border-collapse">
              <thead>
                <tr>
                  {['Id Pedido', 'Cliente', 'Fecha', 'Monto Total', 'Estado', 'Acción'].map((col) => (
                    <th
                      key={col}
                    ...
                    <tr key={p.id} className="hover:bg-[#FAFAFA]">
                      ...
                      <td className="px-4 py-3.5 border-b border-gray-100 align-middle">
                        <button
                          ...
                          onClick={() => navigate(RUTAS.COMERCIANTE_PEDIDO_DETALLE(p.id))}
                        >
                          Ver Detalle
                        </button>
                      </td>
          ...
          {!cargando && filtrados.length > 0 && (
            <div className="flex items-center justify-between px-4 py-3.5 border-t border-gray-200">
              <span className="text-[12px] text-gray-500">
                Mostrando {(paginaActual - 1) * PAGE_SIZE + 1}–{Math.min(paginaActual * PAGE_SIZE, filtrados.length)} de {filtrados.length} pedidos
              </span>
